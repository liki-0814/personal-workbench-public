    ...
    {custom && <Modal title={customId ? '编辑自定义 Provider' : '添加自定义 Provider'} onClose={() => setCustom(null)}><div className="space-y-4 overflow-y-auto p-5">
      ...
      <section className="space-y-3 rounded-xl border border-gray-200 p-4 dark:border-white/10">
        <div className="flex items-center justify-between gap-3">
          <div>
            <p className="text-xs font-medium text-gray-700 dark:text-gray-200">自定义 User-Agent</p>
            <p className="mt-1 text-[11px] text-gray-400">选择预设或直接输入任意 User-Agent。</p>
          </div>
          <span className={`shrink-0 rounded-md px-2 py-1 text-[10px] ${custom.useProxy ? 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-300' : 'bg-gray-100 text-gray-500 dark:bg-white/10 dark:text-gray-400'}`}>{custom.useProxy ? '代理已启用' : '代理未启用'}</span>
        </div>
