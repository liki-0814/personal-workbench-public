...
export default function ProviderModelSettingsPanel() {
  ...
  const applyBatchToolSchemaCompatibility = async (provider: ProviderView, enabled: boolean) => {
    const selectedIds = thinkingSelections[provider.id] ?? new Set<string>();
    if (selectedIds.size === 0) {
      showToast({ message: '请至少选择一个模型', type: 'error' });
      return;
    ...
    const next = {
      ...current,
      models: applyRestrictedToolSchemaCompatibility(current.models, selectedIds, enabled),
    };
    changeDraft(provider.id, () => next);
    await persistDraft(
      provider,
      next,
      enabled
        ? `已为 ${selectedIds.size} 个模型启用受限工具 Schema 兼容`
        : `已为 ${selectedIds.size} 个模型恢复完整工具 Schema`,
    );
  };
          ...
          return <section key={provider.id} className="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-white/10 dark:bg-white/[0.02]">
            ...
            {isExpanded && <div className="border-t border-gray-100 px-4 py-3 dark:border-white/5">
              {advancedProviders.has(provider.id) && <div className="mb-3 rounded-xl border border-purple-200 bg-purple-50/60 p-4 dark:border-purple-500/20 dark:bg-purple-500/[0.06]">
                ...
                <fieldset className="mt-4 border-t border-purple-200 pt-4 dark:border-purple-500/20"><legend className="px-1 text-[11px] font-medium text-gray-600 dark:text-gray-300">工具 Schema 兼容</legend><div className="mt-2 flex flex-wrap items-center justify-between gap-3"><p className="max-w-md text-[10px] leading-5 text-gray-400">仅当供应商提示工具 input_schema 顶层不支持 oneOf、anyOf 或 allOf 时启用。开启后仍会在本地执行前按完整规则校验参数。</p><div className="flex flex-wrap gap-2"><button type="button" disabled={saving === provider.id} onClick={() => void applyBatchToolSchemaCompatibility(provider, false)} className="min-h-10 rounded-lg border border-gray-200 bg-white px-3 py-2 text-xs text-gray-600 hover:bg-gray-50 disabled:opacity-50 dark:border-white/10 dark:bg-white/[0.03] dark:text-gray-300">使用完整 Schema</button><button type="button" disabled={saving === provider.id} onClick={() => void applyBatchToolSchemaCompatibility(provider, true)} className="min-h-10 rounded-lg border border-orange-300 bg-orange-50 px-3 py-2 text-xs font-medium text-orange-700 hover:bg-orange-100 disabled:opacity-50 dark:border-orange-500/40 dark:bg-orange-500/10 dark:text-orange-300">兼容受限 Schema</button></div></div></fieldset>
