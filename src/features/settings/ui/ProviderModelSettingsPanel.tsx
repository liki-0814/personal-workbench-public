          ...
          return <section key={provider.id} className="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-white/10 dark:bg-white/[0.02]">
            ...
            {isExpanded && <div className="border-t border-gray-100 px-4 py-3 dark:border-white/5">
              <div className="divide-y divide-gray-100 dark:divide-white/5">{draft.models.map(model => <div key={model.id} className="flex items-center gap-3 py-3">
                ...
                <div className={`min-w-0 flex-1 ${model.enabled === false ? 'opacity-45' : ''}`}><div className="flex flex-wrap items-center gap-2"><span className="break-all font-mono text-xs text-gray-800 dark:text-gray-100">{model.id}</span>{catalogModelIds.has(model.id) && <span className="rounded bg-gray-100 px-1.5 py-0.5 text-[10px] text-gray-500 dark:bg-white/10">目录</span>}{model.name && model.name !== model.id && <span className="text-[11px] text-gray-400">{model.name}</span>}{model.capabilities?.vision && <span className="rounded bg-blue-500/10 px-1.5 py-0.5 text-[10px] text-blue-600">视觉</span>}{model.capabilities?.thinking && <span className="rounded bg-amber-500/10 px-1.5 py-0.5 text-[10px] text-amber-600">思考</span>}</div>{(model.contextWindow || model.maxOutput) && <p className="mt-1 text-[10px] text-gray-400">{model.contextWindow ? `上下文 ${model.contextWindow.toLocaleString()}` : ''}{model.contextWindow && model.maxOutput ? ' · ' : ''}{model.maxOutput ? `最大输出 ${model.maxOutput.toLocaleString()}` : ''}</p>}</div>
                ...
                {(!provider.builtin || !catalogModelIds.has(model.id)) && <button onClick={() => changeDraft(provider.id, current => ({ ...current, models: current.models.filter(item => item.id !== model.id) }))} className="rounded p-1.5 text-gray-300 hover:bg-red-500/10 hover:text-red-500" aria-label={`删除 ${model.id}`}><Trash2 size={14} /></button>}
              </div>)}</div>
