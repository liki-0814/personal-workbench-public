...
use crate::llm::{ChatMessage, LlmClient, TokenUsage, ToolSchema};
use crate::middleware;
use crate::permissions::PermissionEngine;
#[cfg(test)]
use crate::session::MessageRole;
use crate::session::Session;
use crate::tools::progress::{ImageEmitter, ProgressEmitter};
...
pub trait ToolEventSink: Send + Sync {
    ...
    fn on_decision_started(
        &self,
        _id: &str,
        _trigger: crate::fusion::DecisionTrigger,
        _risk: crate::fusion::DecisionRisk,
    ) {
    }
    fn on_decision_advisor(&self, _id: &str, _model: &str, _succeeded: bool) {}
    fn on_decision_resolved(&self, _id: &str, _verdict: &crate::fusion::DecisionVerdict) {}
    fn on_decision_escalated(&self, _id: &str, _verdict: &crate::fusion::DecisionVerdict) {}
    ...
    /// 长跑工具（如 code_agent）通过 [`crate::tools::progress::emit`] 调用它，
    ...
    /// `generate_image` 工具产出图片时调用 [`crate::tools::progress::emit_image`]，
...
pub struct AgentRunner<'a> {
    ...
    pub background_tasks: Option<&'a std::sync::Arc<crate::background::BackgroundTaskManager>>,
    /// 当前 session_id（后台任务需要关联回 session）
    pub session_id: Option<String>,
    /// Arc 引用的 ToolRegistry，用于后台任务 clone 进 'static future
    pub tool_registry_arc: Option<std::sync::Arc<ToolRegistry>>,
    /// 会话级网页抓取缓存（service 模式注入）
    pub web_cache: Option<std::sync::Arc<crate::tools::web_cache::WebFetchCache>>,
...
impl<'a> AgentRunner<'a> {
    ...
    pub async fn run_turn(
        ...
        let context_window = active_provider
            ...
            .unwrap_or_else(|| crate::usage::context_window_for_model(&active_provider.model));
        let (steer_queue_mode, follow_up_queue_mode) =
            self.harness.map(HarnessControl::queue_modes).unwrap_or((
                crate::harness::QueueMode::OneAtATime,
                crate::harness::QueueMode::OneAtATime,
            ));
        let spec = crate::harness::HarnessSpec::resolve(HarnessInputs {
            ...
            steer_queue_mode: match steer_queue_mode {
                crate::harness::QueueMode::OneAtATime => {
                    crate::harness::spec::QueueModeSpec::OneAtATime
                }
                crate::harness::QueueMode::All => crate::harness::spec::QueueModeSpec::All,
            },
        ...
        let ctx = GraphContext {
            ...
            tool_registry_arc: self.tool_registry_arc.as_ref(),
            web_cache: self.web_cache.as_ref(),
        ...
        if let Err(error) = &run_result {
            ...
            if crate::llm::retry::is_context_overflow(&message) {
                if let Some(harness) = self.harness {
                    harness
                        .events()
                        .emit(crate::harness::HarnessEvent::AutoRetryStart {
                            attempt: 1,
                            max_attempts: 2,
                            delay_ms: 0,
                            error: message.clone(),
                ...
                if let Some(harness) = self.harness {
                    harness
                        .events()
                        .emit(crate::harness::HarnessEvent::AutoRetryEnd {
                            attempt: 1,
...
mod tests {
    ...
    fn make_llm_config(server_url: String) -> RuntimeConfig {
        RuntimeConfig {
            backend_url: server_url.clone(),
            providers: Some(vec![crate::llm::models::ProviderConfig {
                name: "t".into(),
                base_url: server_url,
                api_key: "sk".into(),
            ...
            permissions: crate::config::RuntimePermissionConfig {
                agent_mode: "risk".into(),
    ...
    async fn test_run_turn_no_tool_calls() {
        ...
        let config = make_llm_config(server.url());
    ...
    async fn test_run_turn_single_tool_call_then_text() {
        ...
        let config = make_llm_config(server.url());
        let llm = LlmClient::from_config(&config).unwrap();
        let backend = Arc::new(crate::backend::BackendClient::new(server.url()));
        let mut registry = ToolRegistry::new();
        crate::tools::data_crud::register(&mut registry, Arc::clone(&backend));
        let schemas = registry.to_schemas();
    ...
    async fn test_terminating_tool_ends_without_another_model_call() {
        ...
        registry.register_structured(
            ...
            "terminate the turn",
            serde_json::json!({ "type": "object" }),
            crate::tools::registry::ToolExecutionMode::Parallel,
            Box::new(|_| {
                Box::pin(async {
                    Ok(crate::tools::registry::ToolOutput::terminating(
                        "terminated",
                    ))
                })
            }),
