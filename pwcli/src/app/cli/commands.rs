use crate::app::config::{RuntimeConfig, RuntimeFeatureConfig};
...
- ls: 列出目录内容
- read: 读取文件内容（可用 offset/limit 按行号读片段）
- write: 写入文件
- remove_file: 删除文件/目录
- find: 按 glob 模式查找文件路径
- grep: 按正则搜索文件内容（尊重 .gitignore）
- get_file_info: 获取文件元信息
- bash: 执行 bash 命令（timeout 可选，不传不限时）
- code_agent: 代码相关任务（如有）—— 见下文

## 子 agent: code_agent（仅当本机装有已启用的 Codex / QoderCLI / Kimi Code 时可见）
当用户要求**调研某仓库代码、理解项目结构、找代码 bug、改某个代码项目**时，
优先用 `code_agent` 而不是手动 read + bash 自己摸索。
...
简单任务（看一个 .md、列一个目录、读几行代码）继续用 read / ls，
...
- 数据文件分析：先明确要回答的指标、口径与预期输出列，再用 get_file_info、read 或 bash 检查格式和字段；对 XLSX/CSV/Parquet 等结构化文件，第一次探查应一次性列出相关表的尺寸、表头和字段映射，下一次命令合并完成所有核心取数、清洗、统计与校验。不要按 sheet 或按指标反复发小命令、猜列号；按表头定位并对缺列/短行显式校验。只有出现一个会改变结论的具体未解字段时才追加定向命令。需要图表时生成 PNG，需要可交付物时用 write 保存报告。
...
const FINAL_ANSWER_CONTRACT_NOTE: &str = r##"

## 最终回答契约

- 第一段直接回答用户的问题或给出完成结果；不要从背景、能力介绍或工作过程开始。
- 默认只保留 3–5 个最重要的信息点。背景、边界和实现细节放在结论之后，并仅在有助于用户判断时提供。
...
struct PromptModules {
    visual_generation: bool,
    document_harness: bool,
    data_catalog: bool,
}

fn prompt_modules_for_query(query: Option<&str>) -> PromptModules {
    let query = query.unwrap_or_default().to_lowercase();
    let contains_any = |keywords: &[&str]| keywords.iter().any(|keyword| query.contains(keyword));
    let code_context = contains_any(&[
        "代码",
        "编码",
        "仓库",
        "repo",
    ...
    let create_artifact = contains_any(&[
        "生成",
        "创建",
        "制作",
        "做一份",
    ...
    let edit_artifact = contains_any(&[
        "编辑这份",
        "修改这份",
        "润色这份",
        "更新这份",
    ...
    let document_artifact = contains_any(&[
        "报告",
        "演示",
        "简报",
        "文档",
    ...
    let visual_artifact = contains_any(&[
        "图片",
        "插画",
        "海报",
        "信息图",
    ...
    let explicit_visual_request = contains_any(&["生图", "画一张", "绘制", "draw"]);
    let data_action = contains_any(&[
        "创建", "新增", "列出", "查看", "查询", "更新", "修改", "删除", "完成", "我的", "create",
        "list", "show", "query", "update", "delete", "complete", "my ",
    ]);
    let specific_workbench_domain = contains_any(&[
        "待办",
        "番茄",
        "书签",
        "剪贴板",
    ...
    let ambiguous_workbench_domain = contains_any(&["任务", "目标", "习惯", "goal", "habit"]);
    PromptModules {
        visual_generation: visual_artifact
            && (explicit_visual_request || create_artifact || edit_artifact),
        document_harness: document_artifact
            && (create_artifact || edit_artifact)
            && (!code_context || create_artifact),
        data_catalog: specific_workbench_domain
            || (ambiguous_workbench_domain && data_action && !code_context),
    }
}

fn append_task_prompt_modules(prompt: &mut String, query: Option<&str>) {
    let modules = prompt_modules_for_query(query);
    // Rendering syntax is short and safe for every task. Heavy domain harnesses
    // are injected only when the current user request calls for them.
    prompt.push_str(RICH_RENDERING_NOTE);
    if modules.visual_generation {
        prompt.push_str(VISUAL_GENERATION_POLICY_NOTE);
    }
    if modules.document_harness {
        prompt.push_str(DOCUMENT_HARNESS_NOTE);
    }
}

pub fn append_final_answer_contract(prompt: &mut String) {
    prompt.push_str(FINAL_ANSWER_CONTRACT_NOTE);
}
...
- 用户要求学术方法示意图、基于真实结构化数据的统计图、科学图片润色/编辑或插图验收时，必须先调用 `use_skill({"name":"illustration"})`，再调用统一的 `illustrate` 工具；统计图不得走生图模型。
...
- 分析结构化数据前，先从交付契约派生一张最小取数清单：需要的指标、时间、地域/分组、单位、分母、比较基准和验证规则。对同一个 XLSX/CSV/Parquet，通常只需要一次批量结构探查和一次批量取数计算：首个命令同时输出所有相关 sheet/表的尺寸、候选表头与字段位置；第二个命令按已验证的表头名称提取全部核心完成项、计算派生值并输出校验结果。不要逐 sheet 探测、逐指标重跑、无条件访问猜测的数字列索引，或把一个可合并的分析拆成多次 `bash`。超过两次后，只有明确指出“哪个尚未解决的核心指标、为什么现有输出不足、这条命令将一次补齐什么”才允许继续；否则使用已有证据完成文档并披露边界。
...
- HTML 预留图片位置时使用唯一注释或 `data-placeholder`；资产挂载后用 `patch_document` 的 `replaceText`、`path: "/html"`、`matchText` 和短字符串 `value` 原子替换，不要为插入一张图重传整份 HTML。长文档内容返修先用 `inspect_document_fragments` 获取稳定 `fragmentId`，再用 `replaceFragment` 修改整个标题、段落、列表项、引文或表格单元；`value` 默认只写新的内部 HTML/文字，工具会保留原元素标签和属性，不要重复手抄外层标签。只有确实要改标签或属性时才传同标签的完整元素；删除语义块使用 `remove`、`path: "/html"` 和 `fragmentId`，不要删除 `/html` 根字段。不要在隐藏思考中手工复制长篇旧 HTML。只有零散短语才用 `replaceText`，且 `matchText` 应是能唯一匹配的最短原文。不得通过 `bash` 或直接改 artifacts 文件绕过 revision、校验和原子保存。
...
pub fn append_response_language_policy(prompt: &mut String) {
    let language = crate::app::config::local_config::get().ai.response_language;
    prompt.push_str(response_language_policy_note(language));
}

fn response_language_policy_note(
    language: crate::app::config::local_config::ResponseLanguage,
) -> &'static str {
    use crate::app::config::local_config::ResponseLanguage;
...
pub async fn build_data_catalog(backend: &BackendClient) -> String {
    let registry = crate::runtime::tools::domain_engine::registry();
...
mod response_language_tests {
    ...
    #[test]
    fn task_prompt_modules_are_selected_without_weakening_code_tasks() {
        let code = prompt_modules_for_query(Some("修复 Rust agent loop 并运行测试"));
        assert_eq!(code, PromptModules::default());

        let document_code = prompt_modules_for_query(Some("修复文档渲染代码并完成编码任务"));
        assert_eq!(document_code, PromptModules::default());

        let report = prompt_modules_for_query(Some("生成一份季度报告"));
        assert!(report.document_harness);
        assert!(!report.visual_generation);

        let diagram = prompt_modules_for_query(Some("画一张系统架构图"));
        assert!(diagram.visual_generation);

        let todo = prompt_modules_for_query(Some("列出今天的待办"));
        assert!(todo.data_catalog);
    }
...
pub fn get_system_prompt() -> String {
    ...
    let skill_listing = crate::runtime::skills::build_skill_listing(&skills);
...
fn append_personal_memory_full(buf: &mut String, user_slug: &str) {
    let store = match crate::runtime::memory::MemoryStore::new(user_slug) {
        Ok(s) => s,
        Err(_) => return,
    };
...
fn append_user_profile(buf: &mut String, store: &crate::runtime::memory::MemoryStore) -> bool {
    ...
    let trimmed = truncate_to_max_bytes(
        profile.trim_end(),
        crate::runtime::memory::MAX_PROFILE_BYTES,
    );
...
pub fn memory_injection_opts_from_features(
    ...
    let mem_cfg = crate::app::config::local_config::get().memory;
    let cfg_default = crate::app::config::local_config::MemorySection::default();
...
pub fn last_user_query_from_chat_messages(
    ...
    if !digest.is_empty() {
        ...
        if opts.auto_retrieve {
            if let Some(ref query) = opts.query {
                let q = query.trim();
                if !q.is_empty() {
                    let exclude = crate::runtime::memory::retrieval::digest_slugs(&digest);
...
fn memory_auto_retrieve_hits(
    store: &crate::runtime::memory::MemoryStore,
    query: &str,
    max_results: usize,
    ...
    if index_db.exists() {
        if let Ok(hits) = crate::runtime::memory::hybrid_search(
            store,
            crate::runtime::memory::HybridSearchOptions {
    ...
    crate::runtime::memory::grep_search_top_k(store, query, max_results, exclude, false)
        .ok()
...
pub async fn reindex_user_memory(user_slug: &str) -> anyhow::Result<String> {
    ...
    tokio::task::spawn_blocking(move || {
        let store = crate::runtime::memory::MemoryStore::new(&slug)
            .map_err(|e| anyhow::anyhow!("打开记忆库失败: {}", e))?;
        let embedder = crate::runtime::memory::MemoryEmbedder::new()
            .map_err(|e| anyhow::anyhow!("初始化 embedder 失败: {}（首次需联网下载模型）", e))?;
        let count = crate::runtime::memory::reindex_all(&store, &embedder)
...
pub async fn get_system_prompt_with_catalog(
    ...
    append_task_prompt_modules(&mut prompt, mem_opts.query.as_deref());

    if prompt_modules_for_query(mem_opts.query.as_deref()).data_catalog {
...
pub async fn enhance_system_prompt(
    ...
    enhanced = enhanced.replace(
        ...
        "- 你还有 **bash** 工具可以执行 bash/shell 命令（如 ls、cat、grep、git 等）。",
    );
    ...
    append_task_prompt_modules(&mut enhanced, mem_opts.query.as_deref());
    enhanced
}

fn format_memory_stats(s: &crate::runtime::memory::MemoryStats) -> String {
...
async fn handle_memory_subcommand(parts: &[&str], config: &RuntimeConfig) -> CommandResult {
    ...
    match sub {
        "list" => match crate::runtime::memory::MemoryStore::new(&user_slug) {
        ...
        "show" => {
            ...
            match crate::runtime::memory::MemoryStore::new(&user_slug) {
        ...
        "user" => {
            let source = crate::app::platform::identity::current_identity_source()
                .map(|s| s.label())
                .unwrap_or("unknown");
            CommandResult {
        ...
        "profile" => match crate::runtime::memory::MemoryStore::new(&user_slug) {
        ...
        "stats" => match crate::runtime::memory::MemoryStore::new(&user_slug) {
            Ok(store) => match crate::runtime::memory::collect_stats(&store) {
                Ok(s) => CommandResult {
        ...
        "migrate" => match crate::runtime::memory::MemoryStore::new(&user_slug) {
            Ok(store) => match crate::runtime::memory::migrate_store(&store) {
        ...
        "import-markdown" => {
            ...
            match crate::runtime::memory::MemoryStore::new(&user_slug) {
                Ok(store) => match crate::runtime::memory::import_markdown_tree(&store, &root) {
                    Ok(report) => CommandResult {
                        output: format!(
