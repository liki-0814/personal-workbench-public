...
async fn dispatch(method: &str, params: Value) -> Result<Value> {
    match method {
        ...
        "memory/search" => {
            ...
            let hits = crate::runtime::memory::hybrid_search(
                &current_store()?,
...
pub(crate) async fn extension_host(request: Value) -> Result<Value> {
    let script = materialize_extension_host(&crate::app::config::local_config::data_dir())?;
...
fn current_store() -> Result<crate::runtime::memory::MemoryStore> {
    let config = crate::app::config::RuntimeConfig::load();
    let slug = config
        .user
        .and_then(|user| user.slug)
        .unwrap_or_else(|| "local".to_string());
    Ok(crate::runtime::memory::MemoryStore::new(&slug)?)
}
