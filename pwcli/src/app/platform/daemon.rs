...
use crate::app::config::{local_config, RuntimeConfig};
...
pub async fn run() -> Result<()> {
    ...
    rotate_log_if_needed(&paths.log_path)?;
    crate::app::platform::logging::init_logging_in(&paths.log_dir);
    ...
    let result =
        crate::app::service::run_daemon_server(config, port, runtime, move |registry, backend| {
    ...
    if find_executable("codex-acp").is_none() {
        if let Some(codex) = find_executable("codex") {
            if let Some(check) = acp_clis.first_mut() {
                if find_executable("npm").is_some() {
                    check.ok = true;
                    check.detail = format!(
                        "{} is installed; newest codex-acp adapter will be refreshed via npm before each use",
                        codex.display()
                    );
                } else {
                    check.detail = format!(
                        "{} is installed, but neither codex-acp nor npm was discovered",
                        codex.display()
                    );
                }
            }
