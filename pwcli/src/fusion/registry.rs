...
pub fn resolve_model_ref(
    providers: &[ProviderConfig],
    model_ref: &crate::fusion::config::MoaModelRef,
) -> Option<ProviderConfig> {
...
pub async fn load_providers(backend: &crate::backend::BackendClient) -> Vec<ProviderConfig> {
    ...
    if let Some(providers) = crate::config::RuntimeConfig::load().providers {
