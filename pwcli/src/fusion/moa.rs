...
use crate::fusion::config::{MoaConfig, MoaPreset};
...
use crate::fusion::registry::{is_vision_model, resolve_model_ref};
...
fn localized_decision_system(base: &str) -> String {
    let language_rule = match crate::config::local_config::get().ai.response_language {
        crate::config::local_config::ResponseLanguage::Chinese
        | crate::config::local_config::ResponseLanguage::Auto => {
        ...
        crate::config::local_config::ResponseLanguage::English => {
...
impl MoaRuntime {
    fn provider_for(&self, model: &crate::fusion::config::MoaModelRef) -> Result<ProviderConfig> {
            ...
            .or_else(|| {
                self.providers.iter().find_map(|provider| {
                    provider
                        ...
                        .map(|model| crate::fusion::config::MoaModelRef {
    ...
    fn decision_prompt(
        ...
        let final_review = request.trigger == crate::fusion::DecisionTrigger::FinalReview;
    ...
    async fn review_decision(
        ...
        *self.usage.lock().await = TokenUsage::default();
        ...
        let advisor_limit = if request.trigger == crate::fusion::DecisionTrigger::FinalReview {
            2
        } else {
            match request.risk {
                crate::fusion::DecisionRisk::Low => self.preset.low_risk_advisors.unwrap_or(1),
                crate::fusion::DecisionRisk::Elevated => {
                    self.preset.elevated_risk_advisors.unwrap_or(2)
                }
                crate::fusion::DecisionRisk::High => self
                    .preset
                    .high_risk_advisors
                    ...
                    let result = async {
                        ...
                        for (index, provider) in candidates.iter().enumerate() {
                            ...
                            match response {
                                ...
                                Err(error) => {
                                    let retryable = crate::llm::retry::classify(
                                        None,
                                        &error.to_string(),
                                    ) == crate::llm::retry::ErrorClass::Retryable;
...
pub async fn active_runtime(
    ...
    let config = crate::fusion::load_moa_config(backend).await;
    if config.active_preset.is_empty() {
        return Ok(None);
    }
...
mod decision_tests {
    ...
    fn final_review_treats_requested_placeholders_as_missing_and_sees_data_work() {
        ...
        assert!(is_source_evidence_tool("run_command"));
    ...
    fn fallback_routes_follow_the_active_preset_order() {
        ...
        let runtime = MoaRuntime {
            ...
            preset: MoaPreset {
                advisors: vec![
                    crate::fusion::config::MoaModelRef {
                ...
                judge: crate::fusion::config::MoaModelRef {
