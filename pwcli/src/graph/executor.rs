...
use crate::agent_runner::{PermissionDecision, ToolEventSink, TurnSummary};
...
use crate::middleware::types::{HookAction, ToolCallRequest, ToolResult};
use crate::permissions::{PermissionEngine, PermissionOutcome};
use crate::session::{ContentBlock, ConversationMessage, MessageRole, Session};
use crate::tools::registry::{ToolExecutionMode, ToolRegistry};
...
pub struct GraphContext<'a> {
    ...
    pub permission_engine: &'a PermissionEngine,
    ...
    pub background_tasks: Option<&'a Arc<BackgroundTaskManager>>,
    pub session_id: Option<&'a str>,
    pub tool_registry_arc: Option<&'a Arc<ToolRegistry>>,
    pub web_cache: Option<&'a Arc<WebFetchCache>>,
...
impl AgentGraph {
    ...
    async fn execute_decision_node(
        ...
        if let Some(harness) = ctx.harness {
            harness
                .events()
                .emit(crate::harness::HarnessEvent::DecisionStarted {
        ...
        if let Some(harness) = ctx.harness {
            harness
                .events()
                .emit(crate::harness::HarnessEvent::DecisionResolved {
                    id: pending.request.id.clone(),
        ...
        match verdict.outcome {
            ...
            DecisionOutcome::Escalate => {
                ...
                if let Some(harness) = ctx.harness {
                    harness
                        .events()
                        .emit(crate::harness::HarnessEvent::DecisionEscalated {
    ...
    async fn execute_tool_node(
        ...
        for tc in &tool_calls {
            ...
            if !ctx.config.yolo_mode {
                match ctx.permission_engine.check_call(
                    ...
                    ctx.tool_registry
                        ...
                        .unwrap_or(crate::tools::registry::ToolImpact::ExternalSideEffect),
                ) {
            ...
            if let (Some(bg_mgr), Some(sid), Some(reg_arc)) =
            ...
            if let Some(harness) = ctx.harness {
                harness
                    .events()
                    .emit(crate::harness::HarnessEvent::BeforeTool {
                        id: tc.id.clone(),
            ...
            if let Some(harness) = ctx.harness {
                harness
                    .events()
                    .emit(crate::harness::HarnessEvent::AfterTool {
                        id: tc.id.clone(),
                        name: tc.function.name.clone(),
    ...
    async fn stream_llm_call(
        ...
        'retry: loop {
            ...
            let mut doom_detector =
                crate::middleware::loop_detection::StreamDoomDetector::default();
            ...
            if let Some(ref msg) = error_msg {
                ...
                if !had_output && crate::llm::retry::is_retriable_stream_error(msg) {
                    if let Some(delay) = crate::llm::retry::next_backoff(retry_attempt) {
                        warn!(
...
pub struct GraphBuilder {
    middlewares: Vec<Box<dyn crate::middleware::AgentMiddleware>>,
    config: GraphConfig,
}
...
impl GraphBuilder {
    ...
    pub fn middleware(mut self, m: impl crate::middleware::AgentMiddleware + 'static) -> Self {
        self.middlewares.push(Box::new(m));
        self
    }

    pub fn middlewares(mut self, ms: Vec<Box<dyn crate::middleware::AgentMiddleware>>) -> Self {
        self.middlewares = ms;
        self
    }
...
mod tests {
    use super::{
        ...
        is_configured_background_tool, is_tool_allowed, review_failure_verdict,
        should_review_final, successful_batch_should_terminate, GraphConfig,
        DECISION_REVIEW_TIMEOUT, DOCUMENT_ACTION_TURN_MAX_TOKENS,
    };
    use crate::fusion::{DecisionOutcome, DecisionTrigger, DecisionVerdict};
    use crate::llm::{ChatMessage, FunctionSchema, TokenUsage, ToolSchema};
    ...
    fn tool_results_return_to_agent_without_premature_final_review() {
        ...
        assert!(!should_review_final(
            &state,
            &crate::graph::node::Transition::Goto(crate::graph::node::NodeId::Agent),
            6,
            4,
        ));
    }
    ...
    fn completed_complex_answer_gets_final_review() {
        ...
        assert!(should_review_final(
            &state,
            &crate::graph::node::Transition::End,
            6,
            4,
        ));
    }
    ...
    fn incomplete_document_contract_returns_main_harness_to_agent() {
        ...
        let next =
            continue_incomplete_artifact_contract(&mut state, &crate::graph::node::Transition::End);
        assert!(matches!(
            next,
            Some(crate::graph::node::Transition::Goto(
                crate::graph::node::NodeId::Agent
            ))
        ));
