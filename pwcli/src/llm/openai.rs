...
// `crate::llm::default_max_tokens_for(model)`：Opus → 128K，其他模型（Gemini/
...
fn build_openai_messages_json(messages: &[ChatMessage]) -> Vec<Value> {
    ...
    for m in messages {
        match m.role.as_str() {
            "system" => {
                let (content, _) =
                    crate::middleware::summarization::decode_compaction_summary(&m.content);
...
fn build_openai_request_parts(
    ...
    let deferred_names = messages
        .iter()
        .flat_map(|message| crate::llm::deferred_tools::split(&message.content).0)
    ...
    for message in messages {
        ...
        let (names, _) = crate::llm::deferred_tools::split(&message.content);
        let activated = tools
            .into_iter()
            .flatten()
