...
/// 图像发射器：参数 `(url, alt)`。`generate_image` 工具产出图片时调一次，
/// 让 service `ChannelSink` 把 `StreamEvent::ToolImage` 推到前端 SSE，
/// 前端把 url append 到当前 assistant 消息的 `generatedImages[]`，chat 内联渲染。
pub type ImageEmitter = Arc<
    dyn Fn(&str, &str, Option<&crate::visual_generation::GeneratedImageRecord>)
        + Send
        + Sync
        + 'static,
>;

/// 文本 delta 发射器：长工具在执行阶段需要直接推送正文时，
/// 每个 token delta 通过这个 emitter 上抛，service 转成 StreamEvent::TextDelta 推 SSE，
/// 前端 chat 边收边渲染（不需要等整段 judge 写完）。
...
pub fn emit_generated_image(
    url: &str,
    alt: &str,
    record: &crate::visual_generation::GeneratedImageRecord,
) {
    let _ = CURRENT_IMAGE.try_with(|emitter| {
        if let Some(emitter) = emitter.as_ref() {
            emitter(url, alt, Some(record));
        }
    });
}
