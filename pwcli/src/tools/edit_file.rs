...
pub fn register(registry: &mut ToolRegistry) {
    registry.register_structured_with_impact(
        ...
        Box::new(|value| {
            ...
            Box::pin(async move {
                let args: EditArgs = serde_json::from_value(value).context("Invalid edit_file arguments")?;
...
async fn edit_path(path: &Path, edits: &[Replacement]) -> Result<ToolOutput> {
    ...
    let original = std::str::from_utf8(body).context("edit_file only supports UTF-8 text")?;
