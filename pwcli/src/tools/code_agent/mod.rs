...
fn detect_named_backend(name: &str) -> Option<Box<dyn SubAgentBackend>> {
    match name {
        ...
        "codex" => detect_codex_acp_binary()
            .map(|(bin, _)| Box::new(CodexBackend::new(bin)) as Box<dyn SubAgentBackend>),
...
pub fn list_available_backends(include_models: bool) -> Vec<BackendInfo> {
    ...
    let codex_avail = if include_models {
        detect_codex_acp_binary()
    } else {
        which_binary("codex-acp", &["codex-acp"]).map(|bin| (bin, String::new()))
    };
    if let Some((bin, _)) = &codex_avail {
        let b = CodexBackend::new(bin.clone());
        backends.push(BackendInfo {
        ...
        backends.push(BackendInfo {
            ...
            install_hint: if detect_codex_binary().is_some() {
                "Codex is installed but its ACP adapter is missing; run npm install -g @agentclientprotocol/codex-acp".to_string()
            } else {
...
pub fn resolve_sandbox_root() -> PathBuf {
    let from_cfg = crate::config::local_config::get().tools.fs_base;
        ...
        Box::new(move |args: &serde_json::Value| {
            ...
            Box::pin(async move {
                ...
                let backend: Arc<dyn SubAgentBackend> = match parsed.backend.as_deref() {
                    Some("codex") if backend_is_enabled("codex") => detect_codex_acp_binary()
                        .map(|(bin, _)| Arc::new(CodexBackend::new(bin)) as Arc<dyn SubAgentBackend>)
                        .ok_or_else(|| anyhow::anyhow!("Codex CLI 未安装或 ACP adapter 不可用"))?,
                ...
                let listener: NativeSessionListener = Arc::new(move |native_session_id| {
                    ...
                    crate::tools::progress::emit(&format!(
...
不要用于：
- 读单个 .md 看看（用 read_file）
- 列一个目录（用 list_directory）
