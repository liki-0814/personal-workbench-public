...
pub struct AcpTurnOutput {
    ...
    pub output: String,
    pub usage: Option<crate::tools::code_agent::CodeAgentUsage>,
}
...
async fn run_session_process(
    ...
    let mut command = crate::tools::fs_local::worker_subprocess_command(
        program,
        program_args,
        crate::tools::fs_local::worker_execution_policy(),
        false,
    )?;
    command
        .current_dir(cwd)
...
async fn run_prompt(
    ...
    *active_output
    ...
    let usage = response
        .usage
        .map(|usage| crate::tools::code_agent::CodeAgentUsage {
...
fn bounded_detail(update: &SessionUpdate) -> serde_json::Value {
    ...
    match crate::tools::artifacts::store_tool_output(&serialized) {
