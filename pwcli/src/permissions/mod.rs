...
use crate::tools::registry::ToolImpact;
...
impl PermissionEngine {
    ...
    fn check_call_for_mode(
        ...
        match mode {
            AgentPermissionMode::Full if tool_name != "run_command" => PermissionOutcome::Allow,
            ...
            AgentPermissionMode::Risk if tool_name == "run_command" => {
...
mod tests {
    ...
    fn test_permission_engine_prompt() {
        let engine = PermissionEngine::default_engine();
        assert_eq!(engine.check("write_file"), PermissionOutcome::Prompt);
    }

    #[test]
    fn test_permission_engine_batch() {
        let engine = PermissionEngine::default_engine();
        let names = vec!["list_todos".to_string(), "write_file".to_string()];
