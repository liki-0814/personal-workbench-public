use crate::agent_core::contracts::message::{ContentBlock, ConversationMessage, MessageRole};
use crate::ai::llm::{ChatMessage, FunctionCall, ToolCall};
...
impl Session {
    ...
    /// Project the durable provider transcript into the semantic conversation
    /// exposed to Chat. Tool-call and tool-result messages remain durable for
    /// recovery, but only the last non-tool assistant answer in each user cycle
    /// is part of the normal conversational history.
    pub fn to_public_chat_messages(&self) -> Vec<ChatMessage> {
        fn flush_assistant(
            out: &mut Vec<ChatMessage>,
            fallback: &mut Option<ChatMessage>,
            final_answer: &mut Option<ChatMessage>,
        ) {
            if let Some(mut selected) = final_answer.take().or_else(|| fallback.take()) {
                selected.tool_calls = None;
                selected.tool_call_id = None;
                if !selected.content.is_empty() || !selected.generated_images.is_empty() {
                    out.push(selected);
                }
            }
            *fallback = None;
        ...
        for message in self.to_chat_messages() {
            match message.role.as_str() {
                "user" => {
                    flush_assistant(&mut out, &mut fallback, &mut final_answer);
                    out.push(message);
                }
                "assistant" => {
...
mod tests {
    ...
    fn test_to_chat_messages_roundtrip() {
        ...
        let public = session.to_public_chat_messages();
        assert_eq!(public.len(), 2);
        assert_eq!(public[0].role, "user");
        assert_eq!(public[1].role, "assistant");
        assert_eq!(public[1].content, "当前共 1 篇 wiki");
        assert!(public[1].tool_calls.is_none());
    }
}
