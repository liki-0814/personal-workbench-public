...
fn patch_dangling_tool_calls(messages: &mut Vec<crate::ai::llm::ChatMessage>) -> usize {
    ...
    let mut result: Vec<crate::ai::llm::ChatMessage> = Vec::with_capacity(messages.len());
                ...
                if !tool_calls.is_empty() {
                    ...
                    for tc in tool_calls {
                        if !answered.contains(&tc.id) && !placed.contains(&tc.id) {
                            let content = if tc.function.name == "write" {
...
mod tests {
    ...
    fn write_file_special_message() {
        let mut msgs = vec![
            user("write"),
            assistant_with_tools("", vec![tc("c1", "write")]),
        ];
        patch_dangling_tool_calls(&mut msgs);
        assert_eq!(msgs.len(), 3);
