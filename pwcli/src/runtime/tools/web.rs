...
use crate::runtime::tools::web_cache::WebCacheKey;
...
pub(crate) fn shared_http_client() -> anyhow::Result<reqwest::Client> {
    crate::ai::http::client(
        crate::ai::http::ClientProfile::Web,
        crate::ai::http::ClientFingerprint {
...
pub(crate) async fn cached_fetch_read_with_limit(
    ...
    let content =
        if let Some(hit) = try_cache_get(context, "read", url, offset, Some(&cache_tag)).await {
            ...
            let content = crate::runtime::tools::anysearch::extract(url).await?;
...
fn validate_public_http_url(input: &str) -> anyhow::Result<()> {
    ...
    if host.eq_ignore_ascii_case("localhost")
        ...
        || host
            ...
            .is_ok_and(|ip| !crate::runtime::media::is_public_ip(ip))
    {
...
async fn fetch_public_response(url: &str) -> anyhow::Result<reqwest::Response> {
    ...
    for redirect_count in 0..=3 {
        let (client, checked_url) = crate::runtime::media::restricted_client(&current, false)
            .await
...
async fn download_public_data_file(
    ...
    let directory = crate::runtime::settings::local_config::data_dir()
        .join("artifacts")
        .join("web-downloads")
        .join(&sha256[..16]);
...
pub(crate) async fn cached_fetch_query(
    ...
    let content = if let Some(hit) = try_cache_get(context, "query", &cache_key, 0, None).await {
        ...
        let fresh =
            crate::runtime::tools::anysearch::search_markdown(query, num as usize, site, None)
                .await?;
...
pub(crate) async fn cached_fetch_query_with_hydrate(
    ...
    let items =
        crate::runtime::tools::anysearch::search_items(query, num as usize, site, None).await?;
...
pub fn register(registry: &mut ToolRegistry) {
    ...
    registry.register_structured_with_impact(
        "download_web_file",
        "把已知公开 URL 的二进制数据或资料文件受控下载到 pwcli artifacts，供后续数据分析或文档取证。适合 XLSX、XLS、PDF、ZIP、Parquet，以及需要保留原文件的 CSV/JSON；纯文本正文仍优先 web_read。返回可供 read、read_pdf 或 bash 分析的本地绝对路径、MIME、大小和 SHA-256。只接受 http/https，拒绝本机/私网地址，单文件上限 32 MiB。",
        json!({
            "type": "object",
            "properties": {
                "url": {
    ...
    registry.register(
        ...
        Box::new(|args: &Value| {
            ...
            Box::pin(async move {
                ...
                crate::runtime::tools::anysearch::get_sub_domains(&domains).await
            })
        }),
    );
        ...
        Box::new(move |context, args: &Value| {
            ...
            Box::pin(async move {
                ...
                if !queries.is_empty() {
                    ...
                    if hydrate == 0 {
                        match crate::runtime::tools::anysearch::batch_search_markdown(
                            &queries,
                            num as usize,
                            site.as_deref(),
                            domain.as_deref(),
                ...
                if vertical {
                    let result = crate::runtime::tools::anysearch::search_markdown_with_options(
                        &query,
                        num as usize,
                        site.as_deref(),
                        domain.as_deref(),
...
mod tests {
    ...
    async fn explicit_context_scopes_cache_across_spawn_and_sessions() {
        let cache = std::sync::Arc::new(crate::runtime::tools::web_cache::WebFetchCache::new());
        let context_a = ToolExecutionContext {
            session_id: Some(crate::agent_core::contracts::SessionId::new("session-a")),
        ...
        let context_b = ToolExecutionContext {
            session_id: Some(crate::agent_core::contracts::SessionId::new("session-b")),
            web_cache: Some(std::sync::Arc::clone(&cache)),
            ..ToolExecutionContext::default()
        };
        assert!(
