...
pub struct CodexBackend {
    bin: String,
    refresh: Option<(String, Vec<String>)>,
}
...
impl SubAgentBackend for CodexBackend {
    ...
    fn prepare_command(&self) -> Option<(String, Vec<String>)> {
        self.refresh.clone()
    }
}
...
mod tests {
    ...
    fn codex_adapter_uses_only_acp_transport() {
        ...
        assert!(backend.prepare_command().is_none());
    }
    ...
    fn auto_launcher_refreshes_the_adapter_before_every_call() {
        ...
        assert!(args.contains(&"@agentclientprotocol/codex-acp".to_string()));
