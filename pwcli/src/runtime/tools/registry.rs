use crate::ai::llm::{FunctionSchema, ToolSchema};
use std::sync::{Arc, RwLock};

pub use crate::agent_core::contracts::tool::{
    ToolExecutionMode, ToolImpact, ToolImpactResolver, ToolOutput,
};

use crate::runtime::tools::context::ToolExecutionContext;
use crate::runtime::tools::progress::{self, ImageEmitter, ProgressEmitter};
...
#[async_trait]
impl crate::agent_core::contracts::ports::ToolExecutorPort for ToolRegistry {
    fn has_tool(&self, name: &str) -> bool {
        ToolRegistry::has_tool(self, name)
    }

    fn validate_arguments(&self, name: &str, args: &Value) -> Result<()> {
        ToolRegistry::validate_arguments(self, name, args)
    }

    fn execution_mode(&self, name: &str) -> Option<ToolExecutionMode> {
        ToolRegistry::execution_mode(self, name)
    }

    fn impact_for_call(&self, name: &str, args: &Value) -> Option<ToolImpact> {
        ToolRegistry::impact_for_call(self, name, args)
    }
    ...
    async fn execute(
        ...
        name: &str,
        args: &Value,
        context: crate::agent_core::contracts::ports::ToolInvocationContext,
    ) -> Result<ToolOutput> {
        let base_context = context
            .opaque
            .as_deref()
            .and_then(|value| value.downcast_ref::<ToolExecutionContext>());
