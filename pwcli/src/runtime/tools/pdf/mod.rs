...
use crate::runtime::memory::{MemoryEntry, MemoryIndexLine, MemorySource, MemoryStore};
...
async fn parse_pdf_to_markdown_impl(
    ...
    async fn try_mineru(
        ...
        match mineru::fetch_pdf_via_mineru(url, out_dir).await {
            Ok(md) => {
                let source = if crate::runtime::settings::mineru::get_mineru_token().is_some() {
                    "mineru-precision"
                } else {
                    "mineru-agent"
                };
...
fn remember_pdf_artifact(
    ...
    let config = crate::runtime::settings::RuntimeConfig::load();
...
pub fn backfill_pdf_artifact_memories() -> usize {
    ...
    let root = crate::runtime::settings::local_config::data_dir()
        .join("artifacts")
        .join("pdf");
...
async fn run_read_pdf(
    ...
    let final_dir = artifact_dir(&crate::runtime::settings::local_config::data_dir(), url);
    ...
    match mode {
        ...
        OutputMode::Ephemeral { dir } => format!(
            "\n\n---\n[⏰ 临时模式 | 来源: {src} | 资源目录: {dir} | 24h 后自动清理]\n\
             如需查看图片，调 read 路径 {dir}/images/<filename>（需视觉模型）。",
            src = result.source,
            dir = dir.display(),
        ),
