...
const BUILTIN_ILLUSTRATION_SKILL: &str =
    include_str!("../../../resources/skills/illustration/SKILL.md");
...
fn load_skills_from_disk() -> Vec<Skill> {
    ...
    if !skills.iter().any(|skill| skill.name == "illustration") {
        if let Some(skill) = parse_skill(BUILTIN_ILLUSTRATION_SKILL) {
            skills.push(skill);
        }
    }
    skills
    ...
    for skill in skills {
        if let Some(cmd_template) = skill.command {
            ...
            registry.register_with_impact(
                ...
                crate::runtime::tools::registry::ToolImpact::ExternalSideEffect,
                Box::new(move |args: &Value| {
                    let query = args["query"].as_str().unwrap_or("").to_string();
                    let final_cmd = cmd.replace("{query}", &query);
                    Box::pin(async move {
                        let input =
                            crate::runtime::bash::BashCommandInput::new(final_cmd).with_timeout(30);
                        let sandbox = crate::runtime::bash::SandboxConfig::default();
                        let output = crate::runtime::bash::execute_bash(input, &sandbox).await?;
...
Instructions here."#;
    ...
    #[test]
    fn builtin_illustration_skill_routes_to_the_unified_tool() {
        let skill = parse_skill(BUILTIN_ILLUSTRATION_SKILL).expect("built-in illustration skill");
        assert_eq!(skill.name, "illustration");
        assert!(skill.body.contains("`illustrate`"));
        assert!(skill.body.contains("`archify`"));
        assert!(skill.body.contains("`generate_image`"));
        assert!(skill.body.contains("never use an image model"));
    }
