...
use crate::runtime::tools::code_agent::{
...
fn failure_from_status_and_error(
    ...
    error: Option<&str>,
    attempt: u32,
) -> crate::agent_core::reliability::FailureEnvelope {
    crate::agent_core::reliability::classify_runtime_failure(
        status,
...
fn resolve_execution_spec(spec: &mut RuntimeTaskSpec) -> Option<String> {
    ...
    let mut capabilities = vec![ExecutorCapabilities {
        ...
        native_default_model: inherited_model.or_else(|| {
            crate::runtime::settings::RuntimeConfig::load()
                .active_provider()
    ...
    capabilities.extend(
        crate::runtime::tools::code_agent::list_available_backends(true)
            .into_iter()
            .map(|backend| {
                let efforts = backend
                    .models
...
pub struct RuntimeTaskRecord {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub failure: Option<crate::agent_core::reliability::FailureEnvelope>,
    pub metadata: Value,
}
...
pub fn register_worker_dispatch_tool(
    registry: &crate::runtime::tools::registry::ToolRegistry,
    context: WorkerDispatchContext,
    signal: WorkerDispatchSignal,
) {
    use crate::runtime::tools::registry::{ToolExecutionMode, ToolImpact, ToolOutput};
...
impl TaskBroker {
    ...
    pub fn project_background_tool(
        ...
        let replayable = crate::agent_core::reliability::is_idempotent_read_tool(tool_name);
        let mut connection = self.connection()?;
        let transaction = connection.transaction()?;
        let now = Utc::now().to_rfc3339();
    ...
    pub fn complete_background_tool(
        ...
        if success {
            ...
            let tool_name = payload
                ...
                .to_string();
            let mut envelope = crate::agent_core::reliability::classify_tool_failure(
                &tool_name,
                summary,
                crate::agent_core::reliability::FailureSource::BackgroundTool,
                attempt.saturating_sub(1),
                crate::agent_core::reliability::MAX_AUTO_RETRIES,
            );
            ...
            if !replayable {
                envelope.actions.retain(|action| {
                    !matches!(
                        action.kind,
                        crate::agent_core::reliability::RecoveryActionKind::RetryTask
                    )
                });
                if envelope.actions.is_empty() {
                    envelope
                        .actions
                        .push(crate::agent_core::reliability::RecoveryAction {
                            kind: crate::agent_core::reliability::RecoveryActionKind::OpenSession,
                            label: "打开原会话".into(),
...
pub async fn run_worker(descriptor_path: PathBuf) -> Result<()> {
    ...
    let result: WorkerExecutionResult = async {
        ...
        crate::runtime::tools::fs_local::install_worker_execution_policy(
        ...
        match execution {
            ...
            ResolvedExecution::PwcliAgent {
                ...
                let run = crate::app::cli::application::run_delegated_worker(
                    prompt,
...
验证通过。"#,
    ...
    fn completion_materializes_document_before_batch_callback() {
        ...
        let origin = crate::runtime::documents::DocumentOrigin {
        ...
        let document = crate::runtime::documents::create_delegated_markdown(
            directory.path(),
            "Durable report",
            "# Result\n\nDurable content.",
            origin,
    ...
    fn background_tool_projection_persists_failure_envelope() {
        ...
        assert_eq!(
            failure.class,
            crate::agent_core::reliability::FailureClass::Transient
        );
        assert_eq!(
            failure.disposition,
            crate::agent_core::reliability::FailureDisposition::UserActionRequired
        );
