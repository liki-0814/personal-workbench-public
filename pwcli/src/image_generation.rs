...
use crate::config::local_config::{GenImageModelSection, GenImageSection};
use crate::visual_generation::{
        ...
        let (candidate, qa, final_model, routing_reason) = match initial_qa {
            ...
            Ok(None) => (
                ...
                ImageQaRecord {
                    ...
                    issues: vec![crate::visual_generation::QaIssue {
...
async fn run_vision_qa(
    ...
    let client = crate::llm::LlmClient::with_provider(provider, runtime.backend_url);
    ...
    let message = crate::llm::ChatMessage {
        ...
        content: match response_language {
            crate::config::local_config::ResponseLanguage::Chinese
            | crate::config::local_config::ResponseLanguage::Auto => format!(
                "视觉需求：\n{}\n\n检查附带的生成图片，只返回 JSON。",
                serde_json::to_string(brief)?
            ),
            crate::config::local_config::ResponseLanguage::English => format!(
...
fn qa_system_prompt(language: crate::config::local_config::ResponseLanguage) -> &'static str {
    match language {
        crate::config::local_config::ResponseLanguage::Chinese
        | crate::config::local_config::ResponseLanguage::Auto => QA_SYSTEM_PROMPT_ZH,
        crate::config::local_config::ResponseLanguage::English => QA_SYSTEM_PROMPT_EN,
    }
}
...
fn select_qa_provider(
    ...
    {
        if let Some(provider) = crate::fusion::registry::resolve_model_ref(&providers, model_ref) {
            if crate::fusion::registry::is_vision_model(&providers, &provider.model) {
...
fn emit_stage(stage: VisualStage) {
    crate::tools::progress::emit(&format!("🎨 {}", stage.label()));
}
...
pub(crate) fn is_image_model(model: &Value) -> bool {
    ...
    let id = model
        ...
        .unwrap_or_default()
        .to_ascii_lowercase();
    id.contains("qwen-image") || is_gemini_image_model(id)
}

fn is_gemini_image_model(model_id: impl AsRef<str>) -> bool {
    let model_id = model_id.as_ref().to_ascii_lowercase();
    model_id.starts_with("gemini-") && model_id.contains("image")
}
...
async fn decode_or_fetch_image(source: &str) -> Result<(Vec<u8>, &'static str)> {
    let (bytes, declared_mime) = if let Some(data) = source.strip_prefix("data:") {
        ...
        let image = crate::media::fetch_remote_image(source)
            .await
    ...
    let detected = crate::media::detect_raster_mime(&bytes)
...
```"#,
    ...
    fn lossless_webp_transcode_preserves_dimensions_and_valid_magic() {
        ...
        assert_eq!(crate::media::detect_raster_mime(&webp), Some("image/webp"));
