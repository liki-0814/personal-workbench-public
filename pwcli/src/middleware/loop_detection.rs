...
fn suppress_exhausted_web_access_calls(state: &mut GraphState) -> Vec<String> {
    let exhausted = WEB_ACCESS_TOOLS
        ...
        .map(|tool| crate::graph::state::tool_error_family(tool).to_string())
    ...
    state.pending_tool_calls.retain(|call| {
        !is_web_access_tool(&call.function.name)
            || !exhausted.contains(crate::graph::state::tool_error_family(&call.function.name))
    });
    ...
    async fn after_llm(
            ...
            && tool_names.iter().any(|name| name == "search_file_content")
        {
            ...
            ctx.record_intervention(
                ...
                self.revision(),
                crate::harness::MiddlewareHook::AfterLlm,
                crate::harness::InterventionKind::RequestMutation,
...
mod tests {
    ...
    fn stable_hash_different_calls_different_hash() {
        let c1 = vec![make_tool_call("read_file", r#"{"path":"/a.txt"}"#)];
        let c2 = vec![make_tool_call("read_file", r#"{"path":"/b.txt"}"#)];
    ...
    fn structured_data_guidance_discourages_fragmented_commands() {
        assert!(document_evidence_budget_guidance("run_command", 1)
            .is_some_and(|message| message.contains("batch-extract")));
        assert!(document_evidence_budget_guidance("run_command", 3)
            .is_some_and(|message| message.contains("Stop exploratory command fragmentation")));
        assert!(document_evidence_budget_guidance("run_command", 2).is_none());
        assert!(document_evidence_budget_guidance("run_command", 4).is_none());
    }
