...
impl SummarizationMiddleware {
    ...
    pub fn with_model_context(model: &str) -> Self {
        let window = crate::usage::context_window_for_model(model);
...
pub fn estimate_messages_tokens(messages: &[ChatMessage]) -> u32 {
    messages
        .iter()
        .map(|message| {
            crate::session::estimate_text_tokens(&message.content)
                + message
                    .tool_calls
                    .as_ref()
                    .into_iter()
                    .flatten()
                    .map(|call| crate::session::estimate_text_tokens(&call.function.arguments))
                    .sum::<u32>()
        })
        .sum()
}
        ...
        .flatten()
    {
        ...
        let target = match call.function.name.as_str() {
            "read_file" | "list_directory" | "search_files" | "get_file_info" => {
                &mut traces.read_files
            }
            "write_file" | "edit_file" | "remove_file" => &mut traces.modified_files,
            _ => continue,
        };
...
mod tests {
    ...
    fn with_model_context_sets_70_pct() {
        ...
        let window = crate::usage::context_window_for_model("claude-sonnet-4-6");
    ...
    fn file_traces_accumulate_across_compactions() {
        ...
        assert_eq!(traces.read_files, vec!["old.md"]);
