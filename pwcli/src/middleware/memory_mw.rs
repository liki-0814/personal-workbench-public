...
use crate::memory::compactor::should_auto_compact;
...
impl AgentMiddleware for MemoryMiddleware {
    ...
    async fn after_turn(&self, _state: &mut GraphState, ctx: &GraphContext<'_>) -> Result<()> {
        ...
        if should_auto_compact(&store).unwrap_or(false) {
            let mut tracker = UsageTracker::new();
            let outcome = crate::memory::compactor::maybe_compact_memory(
                &store,
                ctx.llm,
                &mut tracker,
                false,
            ...
            .await;
            match outcome {
                crate::memory::compactor::MemoryCompactOutcome::Done {
                    archived_count,
                    summary_chars,
                } => {
                    info!(
                ...
                crate::memory::compactor::MemoryCompactOutcome::Failed(ref msg) => {
                    warn!(error = %msg, "memory middleware: compaction failed");
                }
                crate::memory::compactor::MemoryCompactOutcome::Skipped(_) => {}
            }
        }
