...
) -> Result<(axum::http::StatusCode, Json<crate::task::TaskBatchAccepted>), ApiError> {
    let session = state
        .session_manager
...
async fn snapshot(
    State(state): State<AppState>,
) -> Result<Json<Vec<crate::task::RuntimeTaskRecord>>, ApiError> {
...
async fn cancel_task(
    State(state): State<AppState>,
    Path(id): Path<String>,
) -> Result<Json<crate::task::RuntimeTaskRecord>, ApiError> {
...
) -> Result<Json<crate::task::TaskDecisionResponse>, ApiError> {
    state
        .task_broker
...
) -> Result<Json<crate::task::TaskReviewResponse>, ApiError> {
    state
        .task_broker
        .review(&id, request)
        .map(Json)
...
struct ReviewDiffResponse {
    ...
    status: String,
    files: Vec<crate::task::worktree::ReviewDiffFile>,
...
fn review_material(
    ...
    (
        crate::task::RuntimeTaskRecord,
        crate::task::worktree::WorktreeLease,
        crate::task::worktree::ReviewArtifact,
    ),
    ApiError,
> {
    let task = state
    ...
    let archived_lease = task
        ...
        .cloned()
        .map(serde_json::from_value::<crate::task::worktree::WorktreeLease>)
        .transpose()
...
async fn review_file(
    ...
    let file =
        crate::task::worktree::read_review_text_file(&lease, &artifact, FilePath::new(&query.path))
            .map_err(bad_request)?;
...
async fn review_diff(
    ...
    let file =
        crate::task::worktree::read_review_diff_file(&lease, &artifact, FilePath::new(&query.path))
            .map_err(bad_request)?;
...
async fn task_documents(
    State(state): State<AppState>,
    Path(id): Path<String>,
) -> Result<Json<Vec<crate::documents::DocumentSummary>>, ApiError> {
    let documents = crate::documents::list(state.task_broker.data_dir())
        .map_err(bad_request)?
        .into_iter()
        .filter(|document| {
            document
    ...
    tokio::spawn(async move {
        ...
        for (session_id, _) in state.session_manager.list() {
            ...
            for queued in recoverable_idle_inputs(&runtime) {
                let task_ids =
                    if queued.source == crate::session::QueuedInputSource::RuntimeCallback {
                        state
                            .session_manager
...
fn handle_document_outbox_failure(
    state: &AppState,
    item: &crate::task::TaskOutboxRecord,
    error: &anyhow::Error,
) -> anyhow::Result<()> {
...
async fn deliver_outbox(
    ...
    if item.destination == "document" {
        ...
        let payload = crate::documents::create_delegated_markdown_or_diagnostic(
            state.task_broker.data_dir(),
            &string("title")?,
            &string("body")?,
            crate::documents::DocumentOrigin {
...
fn apply_review_projection(state: &AppState, payload: &serde_json::Value) -> anyhow::Result<()> {
    ...
    let store = crate::memory::MemoryStore::new(&user_slug)
    ...
    for (index, candidate) in entries.into_iter().enumerate() {
        ...
        let summary = candidate
            ...
            .unwrap_or(content)
            .chars()
            .take(crate::memory::MAX_SUMMARY_CHARS)
            .collect::<String>();
        ...
        let mut entry = crate::memory::MemoryEntry::new_active(
            slug.clone(),
            summary.clone(),
            content.to_string(),
            now,
        ...
        entry.sources.push(crate::memory::MemorySource {
        ...
        store
            ...
            .map_err(|error| anyhow::anyhow!(error.to_string()))?;
        store
            .upsert_index_line(&crate::memory::MemoryIndexLine {
                slug,
                summary,
                updated_at: now,
            })
...
mod callback_actor_tests {
    ...
    fn queued_input(id: &str, source: &str) -> crate::session::QueuedInput {
    ...
    fn daemon_startup_recovers_idle_user_and_callback_inputs() {
        let mut runtime = crate::session::SessionRuntimeSnapshot::idle("session-recovery");
        runtime.queue = vec![
