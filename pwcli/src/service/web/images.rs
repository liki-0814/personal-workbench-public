...
async fn delete_session_artifacts(
    ...
    match crate::image_generation::delete_session_artifacts(data_dir, &session_id).await {
...
async fn serve_artifact(
    ...
    let data_dir = web
        ...
        .parent()
        .unwrap_or_else(|| FilePath::new("."));
    let path = match crate::image_generation::artifact_path(data_dir, &session_id, &artifact_id) {
        Ok(path) => path,
...
async fn fetch_remote(Json(request): Json<FetchRequest>) -> Response<Body> {
    match crate::media::fetch_remote_image(&request.url).await {
        Ok(image) => {
