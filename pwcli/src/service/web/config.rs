...
pub struct ConfigStore {
    repository: crate::config::ConfigRepository,
}

impl ConfigStore {
    pub fn new(data_dir: PathBuf) -> Self {
        Self {
            repository: crate::config::ConfigRepository::new(
                crate::config::RuntimeConfig::config_file(),
                data_dir,
            ),
        }
    }
    ...
    pub fn write_ssh_servers(&self, incoming: Value) -> Result<Value> {
        serde_json::from_value::<Vec<crate::tools::ssh::config::SshServer>>(incoming.clone())
            ...
            .filter_map(|provider| {
                ...
                Some(json!({
                    ...
                    "models": provider.get("models").and_then(Value::as_array).into_iter().flatten()
                        .filter(|model| !crate::image_generation::is_image_model(model))
...
async fn put_config(
    ...
    crate::config::local_config::reload()
        .await
        .map_err(internal)?;
...
fn validate_managed_patch(incoming: &Value) -> Result<()> {
    ...
    if let Some(value) = incoming.pointer("/tools/sshServers") {
        serde_json::from_value::<Vec<crate::tools::ssh::config::SshServer>>(value.clone())
            .context("tools.sshServers must be a valid SSH server array")?;
    }
    if let Some(value) = incoming.pointer("/tools/delegation") {
        serde_json::from_value::<crate::config::local_config::DelegationSection>(value.clone())
...
fn pick_managed(root: &Value) -> Value {
    ...
    output.insert(
        ...
        root.get("schemaVersion")
            ...
            .unwrap_or_else(|| json!(crate::config::CONFIG_SCHEMA_VERSION)),
    );
    copy_section(
        root,
        &mut output,
