...
use super::WebState;
use crate::documents::{CreateDocument, RevisionConflict, UpdateDocument};
...
async fn list(State(state): State<AppState>) -> Response<Body> {
    ...
    match crate::documents::list(data_dir(web)) {
...
async fn create(
    ...
    match crate::documents::create(data_dir(web), request) {
...
async fn read(State(state): State<AppState>, AxumPath(id): AxumPath<String>) -> Response<Body> {
    ...
    match crate::documents::read(data_dir(web), &id) {
        Ok(document) => json_response(
            StatusCode::OK,
            json!({ "success": true, "document": document }),
        ),
...
async fn read_evidence(
    ...
    match crate::documents::read_evidence_ledger(data_dir(web), &id) {
...
async fn update(
    ...
    match crate::documents::update(data_dir(web), &id, request) {
        Ok(document) => json_response(
            StatusCode::OK,
            json!({ "success": true, "document": document }),
        ),
...
async fn remove(State(state): State<AppState>, AxumPath(id): AxumPath<String>) -> Response<Body> {
    ...
    match crate::documents::delete(data_dir(web), &id) {
...
async fn upload_asset(
    ...
    match crate::documents::store_asset(data_dir(web), &id, &filename, &bytes) {
...
async fn serve_asset(
    ...
    let path = match crate::documents::resolve_asset(data_dir(web), &id, &asset) {
        Ok(path) => path,
...
async fn export_html(
    ...
    match crate::documents::export_html(data_dir(web), &id) {
        Ok((manifest, html)) => html_response(
            html,
            Some(&manifest.title),
            manifest.runtime == crate::documents::DocumentRuntime::Archify,
        ),
        Err(error) => failure(StatusCode::NOT_FOUND, &error.to_string()),
    }
}
...
async fn import_html(State(state): State<AppState>, mut multipart: Multipart) -> Response<Body> {
    ...
    match crate::documents::import_html(data_dir(web), &title, &bytes) {
        Ok(document) => json_response(
            StatusCode::CREATED,
            json!({ "success": true, "document": document }),
        ),
