...
fn append_latest_user_message(session: &mut crate::session::Session, messages: &[ChatMessage]) {
...
impl ProviderOverride {
    fn resolve_provider(&self, state: &AppState) -> anyhow::Result<ProviderConfig> {
        ...
        let model_entry = crate::config::provider::ModelEntry {
            id: model.clone(),
            name: model.clone(),
            enabled: None,
            max_output: None,
...
pub struct HarnessControlResponse {
    pub phase: crate::harness::HarnessPhase,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub released_message: Option<ChatMessage>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub released_input: Option<crate::session::QueuedInput>,
    pub runtime: Option<crate::session::SessionRuntimeSnapshot>,
}
...
struct SubmitInputResponse {
    item: crate::session::QueuedInput,
    runtime: crate::session::SessionRuntimeSnapshot,
    effective_delivery: crate::session::QueuedInputDelivery,
}
...
struct PatchQueueItemRequest {
    ...
    delivery: Option<crate::session::QueuedInputDelivery>,
}
...
async fn daemon_status(
...
) -> Result<Json<crate::daemon::DaemonStatus>, axum::http::StatusCode> {
    let runtime = state
        .daemon_runtime
        .as_ref()
    ...
    let active_acp_sessions = crate::tools::code_agent::acp_runner::active_session_count();
    Ok(Json(crate::daemon::DaemonStatus {
        running: true,
...
) -> Json<Vec<crate::harness::HarnessEventRecord>> {
...
async fn session_harness(
    state: &AppState,
    session_id: &str,
) -> std::sync::Arc<crate::harness::HarnessControl> {
        ...
        HarnessAction::Resume => {
            ...
            if let Some(item) = released {
                harness
                    ...
                    .await
                    .map_err(|error| (axum::http::StatusCode::CONFLICT, error.to_string()))?;
                if item.source == crate::session::QueuedInputSource::RuntimeCallback {
...
fn released_message_for_client(item: &crate::session::QueuedInput) -> Option<ChatMessage> {
    (item.source == crate::session::QueuedInputSource::User).then(|| item.message.clone())
}
...
async fn submit_session_input(
    ...
    let requested_delivery = match request.delivery {
        InputDeliveryRequest::Guidance => crate::session::QueuedInputDelivery::Guidance,
        InputDeliveryRequest::Auto if runtime.active_turn_id.is_some() => {
            crate::session::QueuedInputDelivery::NextTurn
        }
    ...
    if runtime.phase == "idle"
        ...
        && runtime.queue.iter().any(|queued| {
            queued.id == item.id
                && queued.status == crate::session::QueuedInputStatus::Queued
                && queued.delivery == crate::session::QueuedInputDelivery::NextTurn
        })
    {
...
async fn get_session_runtime(
    State(state): State<AppState>,
    Path(id): Path<String>,
) -> Result<Json<crate::session::SessionRuntimeSnapshot>, axum::http::StatusCode> {
    state
        .session_manager
        .runtime(&id)
        .map(Json)
...
) -> Result<Json<crate::session::QueuedInput>, (axum::http::StatusCode, String)> {
    let harness = session_harness(&state, &id).await;
    harness
...
async fn list_backends(Query(query): Query<ListBackendsQuery>) -> Json<Vec<serde_json::Value>> {
    let backends = crate::tools::code_agent::list_available_backends(query.include_models);
...
async fn reload_resources(
    ...
    crate::skills::init();
    let skills = crate::skills::load_skills().len();
    let extensions =
        crate::tools::extensions::register_trusted(std::sync::Arc::clone(&state.tool_registry))
            .await
            .map_err(|error| {
                (
                    axum::http::StatusCode::INTERNAL_SERVER_ERROR,
...
async fn list_session_branches(
    State(state): State<AppState>,
    Path(id): Path<String>,
) -> Result<Json<Vec<super::session_manager::BranchEntryInfo>>, (axum::http::StatusCode, String)> {
    state
        .session_manager
        .branches(&id)
        .map(Json)
...
async fn switch_session_branch(
    ...
    let summary = if divergent.trim().is_empty() {
        ...
        let mut usage = crate::usage::UsageTracker::new();
        match crate::llm::summarize_via_llm(
            &divergent,
...
async fn delete_session(
    ...
    crate::bash::shell_state::clear(&id);
...
async fn relocate_session_workspace(
    ...
    let canonical = sandbox
        ...
        .map_err(|error| (axum::http::StatusCode::BAD_REQUEST, error.to_string()))?;
    session.workspace = Some(crate::session::WorkspaceBinding {
        canonical_path: canonical.clone(),
        display_path: std::path::PathBuf::from(&request.path),
        locked_at: Some(chrono::Utc::now()),
    });
    session.state = crate::session::SessionState::Paused;
...
async fn compact_session_context(
    ...
    crate::memory::schedule_before_compaction(
        user_slug,
        session.messages.clone(),
        crate::config::local_config::get()
            .features
            .auto_memory_extract,
    );
    let mut messages = session.to_chat_messages();
    let mut usage = crate::usage::UsageTracker::new();
    match crate::app::compact_session(
        &mut session,
        &mut messages,
        &mut usage,
        &state.config,
    ...
    .await
    {
        ...
        crate::app::CompactOutcome::Skipped(reason) => {
            Err((axum::http::StatusCode::UNPROCESSABLE_ENTITY, reason))
        }
        crate::app::CompactOutcome::Failed(reason) => {
...
async fn chat(
    ...
    let mem_opts = crate::commands::memory_injection_opts_from_features(
        &state.config.features,
        crate::commands::last_user_query_from_chat_messages(&req.messages),
    );
    let mut system_prompt = match req.system_prompt {
        Some(sp) => crate::commands::enhance_system_prompt(&sp, &user_slug, &mem_opts).await,
        None => {
            crate::commands::get_system_prompt_with_catalog(&state.backend, &user_slug, &mem_opts)
                .await
        }
    };
    system_prompt.push_str(&format!(
        "\n\n## 当前工作目录\n\n用户已设置会话工作目录为 `{}`。\n\
                ...
                - 使用 list_directory / read_file / write_file 时优先使用相对于此目录的路径\n\
                - 用户提到的相对路径默认相对此目录",
        bound_cwd.display()
    ));
    crate::commands::append_web_search_note(&mut system_prompt);
    crate::commands::append_response_language_policy(&mut system_prompt);
    ...
    while result.is_ok() {
        ...
        if let Some(item) = session_manager.take_next_queued_input(session_id)? {
            let conversation_message = match item.source {
                crate::session::QueuedInputSource::User => {
                    crate::session::ConversationMessage::new_user(&item.message.content)
                }
                crate::session::QueuedInputSource::RuntimeCallback => {
                    crate::session::ConversationMessage::new_system(&item.message.content)
                }
            };
...
fn queued_input_execution_message(item: &crate::session::QueuedInput) -> ChatMessage {
    let mut message = item.message.clone();
    message.images = item
        .image_urls
        .iter()
        .filter_map(|url| crate::llm::ImageAttachment::from_url(url))
        .collect();
...
fn finalize_turn(state: &AppState, user_slug: String, session: &crate::session::Session) {
    state.session_manager.update(session.clone());
    crate::memory::schedule_after_turn(
        user_slug,
...
struct ChannelSink {
    ...
    permission_broker: std::sync::Arc<crate::permissions::PermissionBroker>,
    session_id: String,
    cwd: String,
    require_permission_approval: bool,
}

impl crate::agent_runner::ToolEventSink for ChannelSink {
    ...
    fn on_decision_started(
        &self,
        id: &str,
        trigger: crate::fusion::DecisionTrigger,
        risk: crate::fusion::DecisionRisk,
    ) {
    ...
    fn on_decision_resolved(&self, id: &str, verdict: &crate::fusion::DecisionVerdict) {
    ...
    fn on_decision_escalated(&self, id: &str, verdict: &crate::fusion::DecisionVerdict) {
        let _ = self.tx.send(StreamEvent::DecisionEscalated {
            id: id.to_string(),
            rationale: verdict.rationale.clone(),
            options: verdict.options.clone(),
        });
    }
    ...
    fn ask_permission<'a>(
        ...
        Box<dyn std::future::Future<Output = crate::agent_runner::PermissionDecision> + Send + 'a>,
    > {
        Box::pin(async move {
            if name == "run_command" {
                let parsed: serde_json::Value = serde_json::from_str(args).unwrap_or_default();
                let cmd = parsed.get("command").and_then(|v| v.as_str()).unwrap_or("");
                match crate::permissions::bash_safety::evaluate(cmd) {
                    crate::permissions::bash_safety::Verdict::Deny(reason) => {
                        return crate::agent_runner::PermissionDecision::DenyWithReason(reason);
                    }
                    crate::permissions::bash_safety::Verdict::Prompt(_) => {}
                    crate::permissions::bash_safety::Verdict::Allow => {
                        if crate::permissions::allow_rule_matches(name, args, &self.cwd) {
                            return crate::agent_runner::PermissionDecision::Allow;
                        }
                    }
                }
            } else if crate::permissions::allow_rule_matches(name, args, &self.cwd) {
                return crate::agent_runner::PermissionDecision::Allow;
            }
            let agent_mode = crate::permissions::current_agent_permission_mode();
            if self.require_permission_approval
                || agent_mode != crate::permissions::AgentPermissionMode::Full
            {
                return self
                    .permission_broker
                    .request(
            ...
            if name == "write_file" || name == "edit_file" || name == "remove_file" {
                return crate::agent_runner::PermissionDecision::Allow;
            }
            if name != "run_command" {
                return crate::agent_runner::PermissionDecision::Deny;
            }
            let parsed: serde_json::Value = serde_json::from_str(args).unwrap_or_default();
            let cmd = parsed.get("command").and_then(|v| v.as_str()).unwrap_or("");
            match crate::permissions::bash_safety::evaluate(cmd) {
                crate::permissions::bash_safety::Verdict::Allow => {
                    crate::agent_runner::PermissionDecision::Allow
                }
                crate::permissions::bash_safety::Verdict::Deny(reason) => {
                    crate::agent_runner::PermissionDecision::DenyWithReason(reason)
                }
                crate::permissions::bash_safety::Verdict::Prompt(reason) => {
                    crate::agent_runner::PermissionDecision::DenyWithReason(reason)
                }
            }
        })
    ...
    fn on_context_usage(&self, usage: crate::llm::TokenUsage, call_index: u32) {
        let _ = self
            .tx
            .send(StreamEvent::ContextUsage { usage, call_index });
    }
    fn progress_emitter_for(&self, id: &str) -> Option<crate::tools::progress::ProgressEmitter> {
    ...
    fn image_emitter_for(&self, id: &str) -> Option<crate::tools::progress::ImageEmitter> {
        let tx = self.tx.clone();
        let id = id.to_string();
        Some(std::sync::Arc::new(move |url: &str, alt: &str, record| {
            let _ = tx.send(StreamEvent::ToolImage {
                ...
                url: url.to_string(),
                alt: alt.to_string(),
                record: record.cloned().map(Box::new),
            });
        }))
    }
}
...
async fn stream_chat(
    ...
    let stream = async_stream::stream! {
        ...
        if let Some(request_cwd) = req.cwd.as_deref().filter(|cwd| !cwd.trim().is_empty()) {
            let request_matches = crate::tools::fs_local::FsSandbox::from_config()
        ...
        let mut system_prompt = match req.system_prompt {
            Some(sp) => crate::commands::enhance_system_prompt(&sp, &user_slug, &mem_opts).await,
            None => {
                crate::commands::get_system_prompt_with_catalog(
                    &state.backend,
                    &user_slug,
                    &mem_opts,
                )
    ...
    let stream = async_stream::stream! {
        loop {
            match rx.recv().await {
                Ok(event) => {
                    let (event_type, data) = match &event {
                        crate::background::TaskEvent::Started { .. } => {
                            ("background_task_started", serde_json::to_string(&event).unwrap_or_default())
                        }
                        crate::background::TaskEvent::Completed(r) => {
                            ("background_task_completed", serde_json::to_string(r).unwrap_or_default())
...
async fn list_background_tasks(
    State(state): State<AppState>,
) -> Json<Vec<crate::background::TaskInfo>> {
...
mod tests {
    ...
    fn runtime_callback_release_is_not_forwarded_as_a_user_message() {
        let callback = serde_json::from_value::<crate::session::QueuedInput>(serde_json::json!({
            "id": "queue-callback",
        ...
        .unwrap();
        let user = serde_json::from_value::<crate::session::QueuedInput>(serde_json::json!({
            "id": "queue-user",
    ...
    fn persisted_turn_is_eligible_for_memory_extraction() {
        ...
        let turn = crate::memory::turn_messages_slice(&session.messages);
        assert!(crate::memory::format_turn_messages(turn).is_some());
    }
