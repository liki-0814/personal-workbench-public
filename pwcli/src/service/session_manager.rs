...
use crate::harness::{HarnessAuditSink, HarnessFingerprint, HarnessSpec, MiddlewareIntervention};
...
impl SessionManager {
    ...
    fn try_append_compaction(
        ...
        existing: &[crate::session::ConversationMessage],
        session: &Session,
    ) -> anyhow::Result<bool> {
        let Some(summary_message) = session.messages.first() else {
            return Ok(false);
        };
        if summary_message.role != crate::session::MessageRole::System
        ...
        let (summary, traces) =
            crate::middleware::summarization::decode_compaction_summary(&encoded_summary);
...
mod tests {
    ...
    async fn harness_audit_is_durable_deduplicated_and_context_neutral() {
        ...
        let spec = crate::harness::HarnessSpec::resolve(crate::harness::HarnessInputs {
            profile: crate::harness::HarnessProfile::Main,
            max_rounds: 100,
        ...
        sink.record_spec(&spec, &fingerprint).await.unwrap();
        sink.record_intervention(&crate::harness::MiddlewareIntervention::new(
            fingerprint,
            "loop_detection",
            1,
            crate::harness::MiddlewareHook::AfterLlm,
            crate::harness::InterventionKind::ControlFlow,
            "test_intervention",
            3,
            None,
    ...
    fn compaction_is_lossless_in_journal_and_materializes_compact_context() {
        ...
        summary.role = crate::session::MessageRole::System;
    ...
    fn queued_user(content: &str) -> crate::llm::ChatMessage {
    ...
    fn idle_guidance_atomically_becomes_next_turn() {
        ...
        let item = manager
            .enqueue_input(
                &id,
                "client-1".into(),
                crate::session::QueuedInputDelivery::Guidance,
                queued_user("continue"),
                Vec::new(),
                Vec::new(),
            )
            .unwrap();
        assert_eq!(item.delivery, crate::session::QueuedInputDelivery::NextTurn);
    ...
    fn runtime_callback_precedes_user_queue_without_crossing_claimed_input() {
            ...
            .unwrap();
        {
            ...
            runtime.queue[0].status = crate::session::QueuedInputStatus::Claimed;
        }
        manager
            .enqueue_input(
                &id,
        ...
        assert_eq!(callback.priority, crate::session::QueuedInputPriority::High);
    }
    ...
    fn paused_runtime_callback_is_immutable_and_consumed_once() {
        ...
        assert_eq!(callback.status, crate::session::QueuedInputStatus::Paused);
        assert!(manager
        ...
        assert_eq!(
            ...
            crate::session::QueuedInputStatus::Claimed
        );
    ...
    fn only_one_turn_can_claim_a_session_and_stop_preserves_queue() {
        ...
        assert_eq!(
            ...
            crate::session::QueuedInputStatus::Paused
        );
    }
    ...
    async fn durable_delivery_conversion_updates_the_active_harness_projection() {
        ...
        manager.mark_turn_started(&id).unwrap();
        let control = crate::harness::HarnessControl::default();
        control.begin(crate::harness::HarnessPhase::Turn).unwrap();
        let item = manager
            .enqueue_input(
                &id,
        ...
        control
            .mutate_durable_queue(Some(&item.id), || {
                manager.replace_queue_item(
                    ...
                    None,
                    Some(crate::session::QueuedInputDelivery::Guidance),
                )?;
        ...
        control
            .mutate_durable_queue(Some(&second.id), || {
                manager.replace_queue_item(
                    ...
                    None,
                    None,
                    Some(crate::session::QueuedInputDelivery::NextTurn),
                )?;
                Ok(((), manager.runtime(&id).unwrap().queue))
            })
            .await
