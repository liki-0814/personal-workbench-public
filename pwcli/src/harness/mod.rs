...
use crate::fusion::{DecisionOutcome, DecisionRisk, DecisionTrigger};
...
impl HarnessControl {
    ...
    pub async fn consume_next_turn(
        ...
        let conversation_message = match queued.source {
            QueuedInputSource::User => {
                crate::session::ConversationMessage::new_user(&queued.message.content)
            }
            QueuedInputSource::RuntimeCallback => {
                crate::session::ConversationMessage::new_system(&queued.message.content)
            }
        };
...
mod tests {
    ...
    async fn consume_next_turn_updates_runtime_and_session_once() {
        ...
        let mut session = crate::session::Session::new("next-turn");
    ...
    async fn priority_next_turn_precedes_user_input_and_dedupes_by_queue_id() {
        ...
        control
            ...
            .unwrap();
        let mut messages = Vec::new();
        let mut session = crate::session::Session::new("priority-next-turn");
