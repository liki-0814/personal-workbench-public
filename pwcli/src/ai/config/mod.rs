...
/// （gpt-image-1、qwen-image-*、gemini-*-image）或 `imagine`（grok-imagine）。
pub fn is_image_generation_model_id(id: &str) -> bool {
    let id = id.to_ascii_lowercase();
    id.contains("image") || id.contains("imagine")
}
...
impl ProviderConfig {
    ...
    pub fn effective_max_tokens(&self, requested: Option<u32>) -> u32 {
        ...
        crate::ai::llm::default_max_tokens_for(&self.model)
    }
}
