...
use super::{
    provider_id, provider_kind, Credential, CredentialStore, OAuthCredential, ProviderKind,
    ProviderRegistry,
};
...
// Public installed-app OAuth client shipped by the MIT-licensed pi-mono
// Antigravity provider through v0.70.5. These values identify the desktop
// client; user access and refresh tokens remain private in CredentialStore.
const DEFAULT_ANTIGRAVITY_CLIENT_ID_B64: &str =
    "MTA3MTAwNjA2MDU5MS10bWhzc2luMmgyMWxjcmUyMzV2dG9sb2poNGc0MDNlcC5hcHBzLmdvb2dsZXVzZXJjb250ZW50LmNvbQ==";
const DEFAULT_ANTIGRAVITY_CLIENT_SECRET_B64: &str =
...
impl AuthManager {
    ...
    pub async fn resolve(&self, provider: &ProviderConfig) -> Result<ResolvedAuth> {
        ...
        if let Some(env_key) = ProviderRegistry::global()
    ...
    pub async fn authenticated_provider(
        ...
        let mut resolved = ProviderRegistry::global().materialize(provider);
...
fn antigravity_oauth_config() -> Result<(String, String)> {
    let runtime = oauth_pair_from_env(
        "GOOGLE_ANTIGRAVITY_CLIENT_ID",
        "GOOGLE_ANTIGRAVITY_CLIENT_SECRET",
    )?;
    if let Some(pair) = runtime {
        return Ok(pair);
    }

    match (
        BUILT_IN_ANTIGRAVITY_CLIENT_ID.filter(|value| !value.trim().is_empty()),
        BUILT_IN_ANTIGRAVITY_CLIENT_SECRET.filter(|value| !value.trim().is_empty()),
    ) {
        (Some(client_id), Some(client_secret)) => {
            return Ok((client_id.to_string(), client_secret.to_string()));
        }
        (Some(_), None) | (None, Some(_)) => {
            anyhow::bail!("Google Antigravity build-time OAuth client configuration is incomplete");
        }
        (None, None) => {}
    }
    ...
    let decode = |value: &str| -> Result<String> {
        let bytes = base64::engine::general_purpose::STANDARD
            .decode(value)
            .context("decode bundled Google Antigravity OAuth client")?;
        String::from_utf8(bytes).context("bundled Google Antigravity OAuth client is not UTF-8")
    };
    Ok((
        decode(DEFAULT_ANTIGRAVITY_CLIENT_ID_B64)?,
        decode(DEFAULT_ANTIGRAVITY_CLIENT_SECRET_B64)?,
    ))
}

fn oauth_pair_from_env(id_key: &str, secret_key: &str) -> Result<Option<(String, String)>> {
    ...
    match (client_id, client_secret) {
        (Some(client_id), Some(client_secret)) => Ok(Some((client_id, client_secret))),
        (None, None) => Ok(None),
        _ => anyhow::bail!(
            "Google Antigravity runtime OAuth client configuration is incomplete; set both {id_key} and {secret_key}"
        ),
    }
}
...
mod tests {
    ...
    #[test]
    fn bundled_antigravity_oauth_client_and_redirect_are_valid() {
        ...
        let client_secret = base64::engine::general_purpose::STANDARD
            .decode(DEFAULT_ANTIGRAVITY_CLIENT_SECRET_B64)
            .unwrap();
        assert!(String::from_utf8(client_id)
            .unwrap()
        ...
        assert!(antigravity_oauth_configured());
    }
}
