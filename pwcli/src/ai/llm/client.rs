...
pub struct LlmClient {
    ...
    auth: std::sync::Arc<crate::ai::provider::AuthManager>,
}

impl LlmClient {
    pub fn from_provider_set(
        active_provider: Option<&str>,
        providers: Option<&[ProviderConfig]>,
        backend_url: impl Into<String>,
    ) -> Result<Self> {
        let backend_url = backend_url.into();
        configure_image_backend_base(&backend_url);
        ...
        let fallbacks: Vec<ProviderConfig> = providers
            .iter()
        ...
        Ok(Self {
            ...
            backend_url,
            session_id: None,
            auth: std::sync::Arc::new(crate::ai::provider::AuthManager::default()),
        })
    }
                ...
                loop {
                    ...
                    match &event {
                        ...
                        StreamEvent::Error(message)
                            if has_fallback
                                && !emitted_output
                                && crate::ai::llm::retry::is_retriable_stream_error(message) =>
                        {
                            let fallback = &providers[index + 1];
                            yield StreamEvent::StreamReset {
                                reason: format!(
