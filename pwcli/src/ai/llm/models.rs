...
pub use crate::ai::config::ProviderConfig;
...
impl ImageAttachment {
    ...
    pub fn from_url(url: &str) -> Option<Self> {
        if url.starts_with("data:") {
            ...
            let backend = image_backend_base()
                .read()
                .expect("image backend base poisoned")
                .clone();
...
fn image_backend_base() -> &'static RwLock<String> {
    static BASE: OnceLock<RwLock<String>> = OnceLock::new();
    BASE.get_or_init(|| RwLock::new("http://localhost:3456".into()))
}

pub(crate) fn configure_image_backend_base(value: &str) {
    *image_backend_base()
        .write()
        .expect("image backend base poisoned") = value.trim_end_matches('/').to_string();
}
...
pub enum StreamEvent {
    ...
    /// user-facing text. `round` is stable for the duration of that call.
    AssistantSegmentStart { round: u32 },
    ...
    ToolImage {
        ...
        url: String,
        alt: String,
        record: Option<Box<serde_json::Value>>,
    },
