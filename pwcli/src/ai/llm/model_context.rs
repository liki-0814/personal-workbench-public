...
pub fn compute_vision_support_for(
    providers: &[crate::ai::config::ProviderConfig],
    model_id: &str,
) -> bool {
    providers.iter().any(|provider| {
        provider.models.iter().any(|model| {
            (model.id == model_id || model.name == model_id)
                && model
                    .capabilities
                    .as_ref()
...
pub fn effort_for_provider(
    provider: &crate::ai::config::ProviderConfig,
    thinking: bool,
) -> Option<String> {
...
mod tests {
    ...
    fn provider_effort_matches_the_effective_request_parameters() {
        use crate::ai::config::{ModelEntry, ProviderConfig};
    ...
    fn model_capability_comes_from_provider_registry() {
        ...
        let providers = vec![crate::ai::config::ProviderConfig {
            name: "test".into(),
            base_url: "http://localhost".into(),
            api_key: "key".into(),
            protocol: "openai".into(),
